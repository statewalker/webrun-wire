...
export async function startFixture(): Promise<Fixture> {
  const server = createServer(async (req, res) => {
    ...
    if (url.pathname.startsWith("/request-stream")) {
      ...
      // connection", which a concatenated result alone cannot distinguish
      // (both produce the same bytes; only the timing differs). The bytes are
      // kept too, and returned alongside the timestamps: the timing proves it
      // streamed, the bytes prove it arrived whole -- a route that only timed
      ...
      const body = Buffer.concat(chunks).toString("utf8");
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify({ arrivals, body }));
      return;
    }
