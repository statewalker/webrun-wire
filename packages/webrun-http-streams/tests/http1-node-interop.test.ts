...
describe("conformance against a real HTTP implementation", () => {
  it("node:http parses a request we generated, and we parse its response", {
    timeout: 15000,
  }, async () => {
    const seen: {
      method?: string;
  ...
  it("we parse a request node:http generated, and node:http parses our response", {
    timeout: 15000,
  }, async () => {
    let decodedMethod = "";
    let decodedUrl = "";
  ...
  it("HEAD: node:http answers our HEAD request, and we read no body from its response", {
    timeout: 15000,
  }, async () => {
    const server = createServer((req, res) => {
    ...
    expect(decoded.envelope.headers.some(([k]) => k.toLowerCase() === "content-length")).toBe(true);
