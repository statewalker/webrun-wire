---
"@statewalker/webrun-streams": patch
"@statewalker/webrun-streams-libp2p": patch
"@statewalker/webrun-http-browser": patch
"@statewalker/webrun-rpc": patch
---

Cancellation now reaches the producer, across every adapter.
...
`emulateMux`'s outbound pump checked `closed` only *after* a chunk arrived, so
the producer leaked. Both acquire the iterator once and cancel without awaiting
now. `framedOutbound` also stopped telling a *server's* handler that its caller
...
base throws `Invalid URL` from a *constructor*, naming neither the option nor
...
A ServiceWorker *is* told when its client aborts: `request.signal` never fires
