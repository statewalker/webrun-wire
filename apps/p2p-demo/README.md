...
- **Server page** — registers one or more HTTP services on a local
  `SiteHandler`, announces them to the group.
...
peer's last-known catalog in that group. The relay authenticates the
announcing peer through the connection's **Noise-proven** identity
(`context.remotePeer`), never from a claim in the payload, and keeps one
...
```sh
E2E_BROWSER=chromium pnpm run e2e   # default if E2E_BROWSER is unset
...
declare function joinGroup(params: {
  node: Libp2p;
  groupId: string;
  relay: Multiaddr;                        // relay's multiaddr — the announce target
}): Promise<GroupHandle>;
```
...
Discovery only tells a page who is in the group; it does not itself open a
connection to anyone. `joinGroup` runs a background **connection pre-warmer**
...
```ts
// What travels over one discovery round trip (lib/discovery.ts): two
// length-prefixed records, the UTF-8 group id and the ServiceAnnouncement
// below. groupId is NOT part of ServiceAnnouncement — it lives only in this
// outer envelope, which is how one relay instance serves many isolated
// groups from one GroupState-per-groupId map.

type ServiceAnnouncement = {
  v: 1;                       // schema version
  peerId: string;             // self — overwritten server-side with the
                               // Noise-proven remotePeer before use
...
type Service = HttpService | PresenceHubService;
...
// The relay itself can appear as a synthesized "always-on" member of every
// group's catalogue (see relay/server.ts) — a presence-only marker with
// nothing mountable.
type PresenceHubService = {
  id: string;
  kind: "presence-hub";
  title: string;
};
...
The reply to an `announce` call is a list of `ServiceAnnouncement`s — the
current catalog of every other peer in that group, as last seen by the relay.
There is no separate topic or subscription; the answer arrives synchronously
in the same stream as the request.
...
// createBrowserLibp2pNode wires the libp2p transport stack (WebSockets,
// WebRTC, Circuit-Relay-v2). It takes `groupId` only so callers don't need
// to special-case the factory — discovery itself is not a libp2p service
// registered here; it's the relay-mediated protocol in lib/discovery.ts.
...
async function getOrOpenHandle(peerId: string): Promise<CallHandle> {
  ...
  // Construct the dial address from the known relay multiaddr. Discovery
  // carries no multiaddrs of its own — the client derives the dial target
  // directly from the relay multiaddr plus the discovered peerId. A limited
  // relay-only connection (opened without the /webrtc segment) rejects
  // custom protocols, so the /webrtc form is mandatory to force the upgrade
...
(non-empty → SERVER, empty → CLIENT) — the relay's own always-on entry
renders as a distinct `HUB` badge instead (see `PresenceHubService` above).
...
- `@libp2p/peer-id` — parses string peer ids (from the discovery reply, and
  from a relay multiaddr's `/p2p/...` component) into the `PeerId` objects
  libp2p's dial APIs expect.
- `@multiformats/multiaddr` — multiaddr parsing and construction (dial
