...
- **Emulated** via `emulateMux` from `webrun-streams` for WS, LiveKit, PeerJS, and MessagePort. Wire format: `[varint streamId][1-byte type][payload]` with types `OPEN / DATA / ACK / END / ERROR / CLOSE`. HTTP/2-style even/odd stream-id allocation. Per-stream receiver-advertised credit backpressure — each side's `OPEN`/`ACK` payload carries a big-endian uint32 grant, a sender may have many frames in flight up to its outstanding credit, and ACKs interleave freely across streams, so cross-stream HoL is eliminated.
...
- New conformance suite `webrun-streams-conformance`'s `describeDuplexAdapter` asserts the contract at L0 (envelope round-trip), L1 (concurrent calls), L2 (half-close), L3 (mid-stream cancellation), L4 (error propagation with serialised stack), L5 (transport teardown). Reference loopback (`makeLoopbackPair`) self-validates the suite. **10/10 scenarios green** against the loopback, `MessageChannel` pair, in-process `emulateMux` pipe, and a real `WebSocketServer`.
  (Update, 2026-09-07: the suite has since grown L6 and now runs 11 scenarios. Four pairs run it in-repo: the two `webrun-rpc` stacks — `emulateMux` and `multiplexPort` + `duplexOverPort` — and two `msgpackCodec` byte-pipe pairs, unlimited and frame-capped. L6 is meaningful only for the `emulateMux` pairs, which have a credit window; for the others it degrades to an integrity check, as each pair's own comment says.)
...
- The legacy `webrun-port-*` and `webrun-http-port` packages remain published and functional through the v1 cutover. ADR-0002's `MessagePort`-as-seam survives operationally for those packages; deletion is scheduled for a follow-up OpenSpec change after cross-workspace consumers (statewalker-apps demos, etc.) migrate.
  (Update, 2026-09-07: that deletion has happened — none of those packages exists in this repository now. `webrun-http-streams` replaced `webrun-http-port`, and the port seam returned in a generalised form in `@statewalker/webrun-rpc`, where `duplexOverPort` runs a `Duplex` over a port. So `Duplex` and port-as-seam are no longer alternatives; the port layer produces the `Duplex`.)
- Native-vs-emulated mux divergence is documented, not hidden. libp2p `Duplex`s have credit-window BP and yamux-level HoL avoidance; emulated `Duplex`s have serial inter-stream ordering at the transport queue level. Both pass the same `Duplex` contract but their throughput / latency characteristics differ.
  (This bullet originally contrasted yamux's credit-window BP with the emulated mux's "per-chunk ACK BP". That contrast no longer holds: the emulated mux now uses receiver-advertised credit too, so the remaining divergence is inter-stream ordering, not the backpressure model.)
